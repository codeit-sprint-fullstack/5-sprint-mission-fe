...
const ProductDetail = ({ product: initialProducts }) => {
  const router = useRouter();
  const [formData, setFormData] = useState({
    name: initialProducts.name,
    description: initialProducts.description,
    price: initialProducts.price,
    tag: "",
    tags: initialProducts.tags,
    images: initialProducts.images,
  });
  ...
  const mutation = useMutation({
    mutationFn: (data) => {
      return axiosClient.patch(`/products/${initialProducts.id}`, data);
    },
    onError: (error, variables, context) => {},
    onSuccess: (res) => {
      router.push(`/items/${initialProducts.id}`);
    },
  });
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex items-center justify-between">
        <div className="text-xl text-[#1F2937] font-bold">상품 수정하기</div>
        <button
